...
class _ResinsListScreenState extends State<ResinsListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return _ResinScaffold(
      title: 'Resinas',
      action: widget.admin ? _HeaderAction(icon: Icons.add, tooltip: 'Adicionar resina', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ResinFormScreen()))) : null,
      child: Column(children: [
        Row(children: [Expanded(child: TextField(onChanged: (value) => setState(() => _query = value), decoration: const InputDecoration(prefixIcon: PextAssetIcon(PextAssets.search, size: 21), hintText: 'Ex: Coextrusão', filled: true, fillColor: Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(11)), borderSide: BorderSide(color: _border))))), const SizedBox(width: 9), Container(width: 48, height: 48, decoration: _card(), child: const Padding(padding: EdgeInsets.all(13), child: PextAssetIcon(PextAssets.filter, size: 21)))]),
...
class _ResinDetailScreenState extends State<ResinDetailScreen> {
  ...
  Widget build(BuildContext context) => _ResinScaffold(
    ...
    action: widget.admin
        ? _HeaderAction(icon: Icons.edit_outlined, tooltip: 'Editar conteúdo', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ResinFormScreen())))
        : _HeaderAction(icon: _favorite ? Icons.favorite : Icons.favorite_border, tooltip: 'Favoritar', onTap: () => setState(() => _favorite = !_favorite)),
    child: Column(children: [
...
class _Tabs extends StatelessWidget { final List<String> tabs; final int selected; final ValueChanged<int> onChanged; const _Tabs({required this.tabs, required this.selected, required this.onChanged}); @override Widget build(BuildContext context) => SizedBox(height: 36, child: Row(children: tabs.asMap().entries.map((entry) => Expanded(child: InkWell(onTap: () => onChanged(entry.key), child: Column(children: [Text(entry.value, overflow: TextOverflow.ellipsis, style: TextStyle(color: entry.key == selected ? _blue : const Color(0xFF6B7280), fontSize: 9, fontWeight: entry.key == selected ? FontWeight.bold : FontWeight.normal)), const Spacer(), Container(height: 2, width: double.infinity, color: entry.key == selected ? _blue : _border)])))).toList())); }
class _HeaderAction extends StatelessWidget { final IconData icon; final String tooltip; final VoidCallback onTap; const _HeaderAction({required this.icon, required this.tooltip, required this.onTap}); @override Widget build(BuildContext context) => Container(width: 31, height: 31, margin: const EdgeInsets.only(right: 7), decoration: BoxDecoration(color: Colors.white, border: Border.all(color: _border), borderRadius: BorderRadius.circular(6)), child: IconButton(padding: EdgeInsets.zero, tooltip: tooltip, onPressed: onTap, icon: Icon(icon, size: 19, color: _blue))); }
